...
module.exports.qrLogin = async function (req, res) {
	console.log(req.accessKey ? req.accessKey : 'keysiz')
	let userInfos = await dbModel.user.findOne({ key: req.accessKey });
	if(userInfos){

    console.log(userInfos, userInfos.email, userInfos.password);
    try {
	    const token = createToken(userInfos._id);
        res.cookie('jwt', token, { httpOnly: true, maxAge: maxAge * 1000 });
        res.redirect('/vote');
    }
    catch (err) {
